...
const EmailOctopusSubscribeForm = ({ formId, onSuccess, buttonText = "Secure Your Registration" }: EmailOctopusSubscribeFormProps) => {
    ...
    // Masterclass Logic: Legacy POST (Working 100%)
    ...
    // Masterclass (Legacy Native)
    return (
        <div className="w-full text-left">
            <iframe name={`eo-legacy-bridge-${formId}`} className="hidden" aria-hidden="true"></iframe>
            <form 
                action="https://emailoctopus.com/lists/2f038f28-ebe1-426f-88ae-c731596742aa/members/embedded/1/post" 
                method="post" 
            ...
            >
                ...
                <div className="space-y-4">
                    <div className="space-y-2">
                        ...
                        <input type="text" name="field_0" required placeholder="Your First Name" className="w-full px-5 py-3.5 bg-slate-900/80 border border-slate-700/50 rounded-2xl text-white focus:ring-2 focus:ring-yellow-500/50 outline-none transition-all placeholder:text-slate-600" />
                    </div>
                    <div className="space-y-2">
                        <label className="text-xs font-bold text-slate-400 uppercase tracking-widest pl-1">Email Address</label>
                        <input type="email" name="email_address" required placeholder="your@email.com" className="w-full px-5 py-3.5 bg-slate-900/80 border border-slate-700/50 rounded-2xl text-white focus:ring-2 focus:ring-yellow-500/50 outline-none transition-all placeholder:text-slate-600" />
                    </div>
                </div>
